...
function renderPokemons(pokemonList) {
  main.innerHTML = ""; // Clear existing Pokémon
  pokemonList.forEach((pokemon) => {
    ...
    div.innerHTML = `
      ...
      <p class="pokemon_p">${pokemon.type.join(", ")}</p>
      <p class="Small_p">Candy count: ${pokemon.candy_count || "N/A"}</p>
      <p class="Small_p">${pokemon.weight}</p>
      <p class="weakness">${pokemon.weaknesses.join(", ")}</p>
...
let sortSelect = document.querySelector("#select2");
...
button.addEventListener("click", (e) => {
  e.preventDefault();
  filterPokemon(input.value.toLowerCase());
});
